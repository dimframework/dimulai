...
func (h *AppHandler) LoadRouters() error {
	// Global Middleware — urutan penting, yang pertama menjadi terluar.
	// Recovery paling luar agar panic di middleware mana pun ikut tertangkap.
	h.router.Use(dim.Recovery(h.logger))

	// ClientIP sebelum apa pun yang membaca IP klien (rate limiting, logging).
